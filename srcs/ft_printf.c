...
/*   Updated: 2024/12/09 17:57:38 by albernar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

int	ft_convert_hex(unsigned int n, char *base)
{
	int	result;
...
size_t	ft_searchparams(char param, va_list args)
{
	if (param == 'c')
		return (ft_putchar_fd(va_arg(args, int), 1));
	else if (param == 's')
		return (ft_putstrr_fd(va_arg(args, char *), 1));
	else if (param == 'p')
